(function(){
  ...
  function log(msg){
    ...
    if(typeof msg === 'string'){
      ...
      else if(/\b(attacks|attack|injected|begins charging|dealt| struck)\b/.test(txt)) cls = 'attack';
  ...
  const healingIds = ['heal','hot','charge-heal','team-heal'];
      ...
      enemy.selectedAbilities = chosen.slice(0,3).map(a=>a.id);
      picks.push(enemy);
    }
  ...
  function updateBattleSprites(){
    ...
    try{
      ...
      if(activeEnemy){
        if($enemyName) { $enemyName.textContent = activeEnemy.name || ''; $enemyName.style.color = activeEnemy.isBoss ? '#f0f' : ''; }
  ...
  function applyEffects(target){
    ...
    if(t.hp <= 0){
      ...
      if(playerTeam.includes(t)){
        t.dead = true;
        if(state.player === t){
          const next = playerTeam.find(p=>!p.dead && p !== t);
      ...
      } else if(enemyTeam.includes(t)){
        t.dead = true;
        if(state.enemy === t){
          const next = enemyTeam.find(p=>!p.dead && p !== t);
  ...
  function animateSprite(actorKey, type, size='small'){
    ...
    if(!el) return;
    let clsBase = '';
    if(type === 'attack') clsBase = 'attack-bounce';
    else if(type === 'heal') clsBase = 'heal-bounce';
    else if(type === 'switch') clsBase = 'switch-bounce';
    else if(type === 'team-attack') clsBase = (actorKey === 'player') ? 'team-attack-right' : 'team-attack-left';
    else if(type === 'team-heal') clsBase = 'team-heal-up';
    if(!clsBase) return;
  ...
  const id = filteredAvail.length ? filteredAvail[Math.floor(Math.random()*filteredAvail.length)] : 'attack';
    const ability = ABILITIES.find(a=>a.id===id);
    log(`${actor.name} prepares ${ability? ability.name : id}...`);
    setTimeout(()=>{
      switch(id){
